...
class MBotOLED:
    ...
    def check_mbot_lcm_installed(self):
        try:
            ...
            self.mbot_balbot_feedback_t = mbot_balbot_feedback_t
            return True
        except ImportError:
    ...
    def battery_info_callback(self, channel, data):
        if self.mbot_lcm_installed:
            battery_info = self.mbot_balbot_feedback_t.decode(data)
    ...
    def main_loop(self):
        ...
        if self.lc:
            self.lc.subscribe("MBOT_BALBOT_FEEDBACK", self.battery_info_callback)
